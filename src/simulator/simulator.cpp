...
void Simulator::runGraphicStep2()
{
    ...
    if( deltaRefTime >= 1e9 )               // We want steps per Sec = 1e9 ns
    {
        ...
        CircuitWidget::self()->setRate( (stepsPerSec*100)/(1e4*m_stepsPerus) /*m_stepsPS*/ );
        m_lastStep    = m_tStep;
...
if( m_step == 0 )
{
    ...
    for( uint64_t i=0; i<m_stepsPF; i++ )
    {
        m_circTime = m_step*m_stepNano;         // Circuit Time in nanoseconds

        if( m_runMcu ) BaseProcessor::self()->simuClockStep();   // Run MCU
        ...
        {
            m_reacCounter += m_stepsReac*m_stepsPerus;
...
void Simulator::runExtraStep( uint64_t cycle )
{
    ...
    //qDebug() <<"\nSimulator::runExtraStep"<<m_circTime<<m_step<<m_stepNano;
