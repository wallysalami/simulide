...
void eI2C::simuClockStep()       // We are in Mater mode, controlling Clock
{
    ...
    bool clkLow = m_clockSource->getVolt() < m_inputLowV;

    if( m_toggleClock )
    {
        if( clkLow ) m_clockSource->setImp( high_imp );
        else         m_clockSource->setImp( m_outImp );
        m_toggleClock = false;
    }
    ...
    if( m_state == I2C_STARTED )                    // Send Start Condition
    {
        ...
        {
            //m_clockSource->setOut( false );
            m_clockSource->setImp( m_outImp );
    ...
    else if( m_state == I2C_STOPPED )                // Send Stop Condition
    {
        ...
        else if( !m_SDA && clkLow ) m_clockSource->setImp( high_imp ); // Step 2: Raise Clock
