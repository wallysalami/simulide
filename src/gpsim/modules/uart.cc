...
public:
    ...
    ~RCSignalSource() { }
    virtual char getState() { return m_rcsta->getState(); }
    virtual void release() { m_rcsta->releasePin(); }
...
//-----------------------------------------------------------
_RCSTA::_RCSTA(Processor* pCpu, const char* pName, USART_MODULE* pUSART)
    : SfrReg(pCpu, pName ),
      rcreg(0), spbrg(0),
...
_RCREG::_RCREG( Processor* pCpu, const char* pName,  USART_MODULE* pUSART )
    : SfrReg(pCpu, pName ), fifo_sp(0),  mUSART(pUSART), m_rcsta(0)
{
    assert(mUSART);
}

_BAUDCON::_BAUDCON(Processor* pCpu, const char* pName)
    : SfrReg(pCpu, pName )
{
}

_SPBRG::_SPBRG(Processor* pCpu, const char* pName )
    : SfrReg(pCpu, pName ),
...
_SPBRGH::_SPBRGH( Processor* pCpu, const char* pName  )
    : SfrReg( pCpu, pName  ), m_spbrg(0)
{
}
...
// my reading of the spec is this happens at the end of the next pic instruction.
// If the shift register is empty the bit will then be set one pic instruction later.
// Otherwise the bit is set when the shift register empties.  RRR 10/2014

_TXREG::_TXREG( Processor* pCpu, const char* pName,  USART_MODULE* pUSART )
    : SfrReg( pCpu, pName  ), m_txsta(0), mUSART(pUSART)
{
    assert( mUSART );

    m_uartN = QString::fromStdString( name_str ).remove("txreg").toInt();
    if( m_uartN > 0 ) m_uartN--;
    //qDebug() << "TXREG " << QString::fromStdString( name_str )<<m_uartN;
}

void _TXREG::put( uint new_value )
{
    value.put( new_value & 0xff );
    if( cpu->isHalted() ) return; // Resetting: new_value=0 coming from reset registers.
...
{
    cout << "TXREG " << name_str << " CallBack ID " << CallBackID << '\n';
}

//-----------------------------------------------------------
//-----------------------------------------------------------

_TXSTA::_TXSTA( Processor* pCpu, const char* pName,  USART_MODULE* pUSART )
    : SfrReg( pCpu, pName  )
{
    mUSART = pUSART;
    txreg = 0;
...
// TXSTA - setIOpin - assign the I/O pin associated with the the transmitter.
...
void _TXSTA::setIOpin( PinModule* newPinModule )
{
    ...
    else if( m_PinModule) disableTXPin();  // disconnect old pin

    m_PinModule = newPinModule;
    if(bTXEN() && rcsta->bSPEN()) enableTXPin();
}
...
void _TXSTA::put( uint new_value )
{
    ...
    if( (old_value^value.get()) & TXEN)
    {
        ...
        // has some data that needs to be sent, then start a transmission.
        // If the transmitter is being disabled, then abort any transmission.
...
void _TXSTA::start_transmitting()
{
    ...
    if( cpu ) cpu->setBreakAbs( spbrg->get_cpu_cycle( 1 ), this );
...
{
    cout << "TXSTA " << name_str << " CallBack ID " << CallBackID << '\n';
}
...
void _RCSTA::put( uint new_value )
{
    ...
    if( diff & SPEN && !(new_value & SPEN)) // If SPEN being turned off, clear all readonly bits
    {
        readonly = 0;
    ...
    else if( diff & CREN && !(new_value & CREN)) readonly &=( RX9D | FERR);
    ...
    if( !txsta->bSYNC()) // Asynchronous case
    {
        if( diff &( SPEN | CREN)) // RX status change
        {
            ...
            else if( (value.get() &( SPEN | CREN)) ==( SPEN | CREN))
            {
                ...
                if( m_cRxState == '0' || m_cRxState == 'w') // If rx line is low start receiving now.
                    receive_start_bit();

                value.put( value.get() &( ~OERR) ); // Clear overrun error when turning on RX
            }
            else                 // RX off, check TX
            {
                ...
                {
                    if( txsta->bTXEN()) txsta->enableTXPin();  // TX output active
                    else                txsta->disableTXPin(); // TX off
                }
                return;
            }
        }
    ...
    else                // synchronous case
    {
        ...
        {
            if( (value.get() &( SPEN | SREN | CREN)) == SPEN) // Synchronous transmit( SREN & CREN == 0)
            {
                enableRCPin( DIR_OUT );
                if( txsta->bTXEN()) txsta->enableTXPin();
                return;
            }
            else if( value.get() &( SPEN)) // Synchronous receive( SREN | CREN != 0)
            {
                enableRCPin( DIR_IN );
                txsta->enableTXPin();
                rsr = 0;
...
void _RCSTA::putRCState( char newRCState ) // Update the state of the DTx output pin only used for Synchronous mode
{
...
void _RCSTA::setIOpin( PinModule* newPinModule ) // Assign the I/O pin associated with the the receiver.
{
    if( m_sink )
    {
...
void _RCSTA::sync_start_transmitting() // Transmit in synchronous mode
{
    assert(txreg);
...
void _RCSTA::clock_edge( char new3State )
{
    ...
    state = mUSART->baudcon.txckp() ? !state : state; // invert clock, if requested

    if( old_clock_state == state ) return;
    old_clock_state = state;

    if( !(value.get() & SPEN) ) return;

    if( (value.get() & (CREN | SREN)) == 0 ) // Transmitting
    {
...
void _RCSTA::receive_a_bit(uint bit)
{
    ...
    Dprintf(("%s receive_a_bit state:%u bit:%u time:0x%" PRINTF_GINT64_MODIFIER "x\n",
             name_str.c_str(), state, bit, cpu->get_cycles()->get()));
...
void _RCSTA::set_callback_break( uint spbrg_edge )
{
    ...
    if( cpu && spbrg )
    {
        time_to_event = ( spbrg->cyclesPerTick()*spbrg_edge ) / TOTAL_SAMPLES;
        cpu->setBreakRel( time_to_event, this );
    }
}
...
void _RCSTA::receive_start_bit()
{
    ...
    if((value.get() &( CREN | SREN)) == 0)  return;

    if(txsta &&( txsta->value.get() & _TXSTA::BRGH))
        set_callback_break( BRGH_SAMPLE_1 );
    else
        set_callback_break( BRGL_SAMPLE_1 );
...
void _RCSTA::queueData( uint32_t value ) // Used by Simulide
{
    ...
    if( !dataInQueue ) set_callback_break( spbrg->cyclesPerTick()*9 );
    dataInQueue = true;
}
...
{
    Dprintf(("RCSTA callback. %s time:0x%" PRINTF_GINT64_MODIFIER "x\n", name_str.c_str(), cpu->get_cycles()->get()));

    if( !m_dataQueue.isEmpty() )
    {
        rcreg->push( m_dataQueue.takeFirst() );
        
        if( !m_dataQueue.isEmpty() )  set_callback_break( spbrg->cyclesPerTick()*9 );
    ...
    if( txsta->bSYNC() )        // Synchronous mode RX/DT is data, TX/CK is clock
    {
        if( sync_next_clock_edge_high )        // + edge of clock
        {
            ...
            if( (value.get() &( SPEN | SREN | CREN)) == SPEN) // Transmit
            {
                if( bit_count)
                {
                    putRCState((rsr & 1) ? '1' : '0');
        ...
        else        // - clock edge
        {
            ...
            if( (value.get() &( SPEN | SREN | CREN)) != SPEN) // Receive Master mode
            {
        ...
        {
            future_cycle = cpu->currentCycle() + spbrg->cyclesPerTick();
            cpu->setBreakAbs( future_cycle, this );
        }
    }
    else // A bit is sampled 3 times.
    {
        switch( sample_state )
        {
        case RCSTA_WAITING_MID1:
            ...
            if(txsta &&( txsta->value.get() & _TXSTA::BRGH))
                set_callback_break(BRGH_SAMPLE_2 - BRGH_SAMPLE_1);
            else
                set_callback_break(BRGL_SAMPLE_2 - BRGL_SAMPLE_1);
        ...
        case RCSTA_WAITING_MID2:
            ...
            if( txsta && (txsta->value.get() & _TXSTA::BRGH))
                 set_callback_break( BRGH_SAMPLE_3-BRGH_SAMPLE_2);
            else set_callback_break( BRGL_SAMPLE_3-BRGL_SAMPLE_2);
        ...
        case RCSTA_WAITING_MID3:
            ...
            if( state==RCSTA_RECEIVING ) // If this wasn't the last bit, set a break for the next bit.
            {
                if( txsta && (txsta->value.get() & _TXSTA::BRGH) )
                     set_callback_break( TOTAL_SAMPLES-(BRGH_SAMPLE_3-BRGH_SAMPLE_1) );
                else set_callback_break( TOTAL_SAMPLES-(BRGL_SAMPLE_3-BRGL_SAMPLE_1) );
...
{
    cout << "RCSTA " << name_str << " CallBack ID " << CallBackID << '\n';
}
...
void _SPBRG::setNextBreak()
{
    future_cycle = last_cycle + cyclesPerTick();

    if( cpu )
    {
        if( future_cycle <= cpu->currentCycle() )
        {
...
uint _SPBRG::cyclesPerTick()
{
    uint cpi =( cpu) ? cpu->get_ClockPerInstr() : 4;
    uint brgval, cpt, ret;
    ...
    {
        if( txsta && !(txsta->value.get() & _TXSTA::BRGH) ) cpt *= 4; // lo-speed divisor is 4 times hi-speed
    }
...
void _SPBRG::start()
{
    ...
    if( !skip  || cpu->currentCycle() >= skip )
    {
        if( cpu ) last_cycle = cpu->currentCycle();
        skip = 0;
    }
    running = true;
...
uint64_t _SPBRG::lastCycle() // Get the cpu cycle corresponding to the last edge of the SPBRG
{
    // There's a chance that a SPBRG break point exists on the current
    // cpu cycle, but has not yet been serviced.
    if( cpu ) return( (cpu->currentCycle() == future_cycle) ? future_cycle : last_cycle);
    else    return 0;
}
//--------------------------
...
uint64_t _SPBRG::get_cpu_cycle( uint edges_from_now )
{
    // There's a chance that a SPBRG break point exists on the current
    // cpu cycle, but has not yet been serviced.
    uint64_t cycle = lastCycle();

    return(  edges_from_now*cyclesPerTick() + cycle );
}
...
void _SPBRG::callback()
{
    ...
    {
        // If the serial port is enabled, then set another break point for the next clock edge.
        setNextBreak();
    }
    else running = false;
}
...
USART_MODULE::USART_MODULE( Processor* pCpu )
    : txsta( pCpu,"", this ),    // Don't set names incase there are two UARTS
      rcsta( pCpu,"", this ),
      spbrg( pCpu,"" ),
      spbrgh( pCpu,"spbrgh" ),
      baudcon( pCpu,"baudcon" ),
...
void USART_MODULE::initialize( PIR* _pir, PinModule* tx_pin, PinModule* rx_pin, _TXREG* _txreg, _RCREG* _rcreg )
{
