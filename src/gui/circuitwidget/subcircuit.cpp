...
void SubCircuit::initSubcircuit()
{
    ...
    while( !rNode.isNull() )
    {
        ...
        if( tagName=="item" )
        {
            ...
            eElement* ecomponent = getComponent( &element );
...
eElement* SubCircuit::getComponent( QDomElement* element )
{
    ...
    else if( (type == "eCounter") || (type == "eBinCounter") )
    {
        ...
        bool rInv = true;
        if( element->hasAttribute("resetInverted" ) )
        {
            if( element->attribute( "resetInverted" ) == "false" ) rInv = false;
        }
        ecounter->setResetInv( rInv );
    }
...
void SubCircuit::attach()
{
    ...
    {                                       // and assing to connected subcircuit ePins
        ...
        //qDebug() << "\nSubCircuit::attach() Pin"<< QString::fromStdString( m_ePin[i]->getId() )<< enod->itemId();
        for( ePin* epin : m_pinConections[i] )
        {
            //if( epin->getEnode() ) break;
            //qDebug() << "SubCircuit::attach()"<< QString::fromStdString( epin->getId() );
            epin->setEnode(enod);
        }
    }
}
