          ...
          : Chip( parent, type, id )
{
    ...
    m_hideAction = new QAction( tr("Hide Non Graphic"), this );
...
void SubPackage::contextMenuEvent( QGraphicsSceneContextMenuEvent* event )
{
    ...
    for( Pin* pin : m_pins )
    {
        ...
        //qDebug() << "SubPackage::contextMenuEvent" <<pin->y()<<yPos<<pin->x()<< xPos<<angle;
        if(( yPin == yPos ) && ( xPin == xPos ) )
        {
            m_eventPin = pin;
            break;
        }
...
void SubPackage::contextMenu( QGraphicsSceneContextMenuEvent* event, QMenu* menu )
{
    ...
    else
    {
        ...
        m_hideAction->setChecked( m_hideGrComp );
        menu->addAction( m_hideAction );
        connect( m_hideAction, SIGNAL( triggered()), this, SLOT( hideNonGraphic() ) );

        QAction* mainCompAction = menu->addAction( QIcon(""),tr("Select Main Component") );
        connect( mainCompAction, SIGNAL( triggered()), this, SLOT( mainComp() ) );

        QAction* cbAction = menu->addAction( QIcon(""),tr("Create Board") );
        connect( cbAction, SIGNAL( triggered()), this, SLOT( createBoard() ) );
        menu->addSeparator();
...
void SubPackage::hideNonGraphic()
{
    m_hideGrComp = m_hideAction->isChecked();

    QList<Component*>* compList = Circuit::self()->conList();
    for( Component* comp : *compList )
    {
        Connector* con =static_cast<Connector*>( comp );
        if( con ) con->setVisib( !m_hideGrComp );
    }
    ...
    for( Component* comp : *compList )
    {
        ...
        //qDebug() << "SubPackage::hideNonGraphic"<<comp->objectName()<<comp->itemType();
        //if( comp->isGraphical() ) comp->setHidden( true );
        //else                      comp->setVisible(false);
        comp->setHidden( m_hideGrComp );
    }
    Circuit::self()->update();
}

void SubPackage::createBoard()
{
