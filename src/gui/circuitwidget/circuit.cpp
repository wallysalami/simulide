...
Pin* Circuit::getConPin( QString pinId, QHash<QString, QString>* idMap )
{
    Pin* pin = 0l;
    QString compName = Circuit::self()->getCompId( pinId );
    QString newName  = idMap->value( compName );
...
void Circuit::loadDomDoc( QDomDocument* doc )
{
    ...
    if( circuit.hasAttribute( "speed" ))
    {
        ...
        m_simulator.m_stepNano = 1e6*1000/sps;
        setCircSpeed( 1e6*1000/m_simulator.m_stepNano );
    }
    ...
    if( circuit.hasAttribute( "Simu_Step_nS" )) setStepNs( circuit.attribute("Simu_Step_nS").toInt() );
...
void Circuit::circuitToDom()
{
    ...
    circuit.setAttribute( "Simu_Step_nS", QString::number( stepNs() ) );
...
void Circuit::removeItems()                     // Remove Selected items
{
    ...
    bool pauseSim = m_simulator.isRunning();
    if( pauseSim ) m_simulator.pauseSim();
    ...
    if( pauseSim ) m_simulator.runContinuous();
}
...
void Circuit::undo()
{
    ...
    bool pauseSim = m_simulator.isRunning();
    if( pauseSim ) m_simulator.stopSim();
...
{
    double step = 1e9/rate;
    if( step < m_simulator.m_stepNano ) m_simulator.m_stepNano = step;
...
double Circuit::circSpeedP()
{
    ...
    double sps100  = 1e6*1000/m_simulator.m_stepNano;
...
void Circuit::setCircSpeedP( double speedP )
{
    ...
    double rate = 1e9*speedP/100/m_simulator.m_stepNano;

    setCircSpeed( rate );
}

double Circuit::stepNs() { return m_simulator.m_stepNano; }

void Circuit::setStepNs( double ns )
{
    double spsPc = circSpeedP();
    m_simulator.m_stepNano = ns;
    uint64_t rate = (1e6*1000/ns)*spsPc/100;
