		...
		void * param)
{
    ...
    if (!start) avr_raise_interrupt(avr, &p->overflow);
	p->tov_base = when;
		...
		avr_timer_t * p)
{
	...
	if (!(p->ext_clock_flags & (AVR_TIMER_EXTCLK_FLAG_TN | AVR_TIMER_EXTCLK_FLAG_AS2)) ||
        ...
        if (p->tov_top) return p->tov_base;
	}
	return 0;
}
		...
		void * param)
{
    ...
    if (p->r_tcnth) avr->data[p->r_tcnth] = tcnt >> 8;
		...
		void * param)
{
    ...
    switch (timer->wgm_op_mode_kind) {
		...
		case avr_timer_wgm_fast_pwm:
            if (oldv != _timer_get_comp_ocr(avr, comp)) avr_timer_reconfigure(timer, 0);
            avr_raise_irq(timer->io.irq + TIMER_IRQ_OUT_PWM0, _timer_get_ocr(timer, AVR_TIMER_COMPA));
            avr_raise_irq(timer->io.irq + TIMER_IRQ_OUT_PWM1, _timer_get_ocr(timer, AVR_TIMER_COMPB));
			break;
		...
		void * param)
{
    ...
    if (p->r_icrh) avr->data[p->r_icrh] = tcnt >> 8;
	avr_raise_interrupt(avr, &p->icr);
}
