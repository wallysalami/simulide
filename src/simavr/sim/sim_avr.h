...
typedef struct avr_t {
    ...
    uint32_t vcc,avcc,aref; // (optional) voltages in millivolts mostly used by the ADC for now

    // cycles gets incremented when sleeping and when running; it corresponds
    // not only to "cycles that runs" but also "cycles that might have run" like, sleeping.
    ...
    void (*init)(struct avr_t* avr);  // called at init time
    void (*reset)(struct avr_t* avr); // called at reset time
    ...
    struct {
        ...
        void (*deinit)(struct avr_t* avr, void* data);// called at termination time ( to clean special initializations)
        void *data; // value passed to init() and deinit()
    } custom;
